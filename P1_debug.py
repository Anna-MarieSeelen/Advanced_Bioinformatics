...
Debugging author: Anna-Marie Seelen
Studentnr: 1008970
...
def parse_fasta(lines):
    ...
    seqs = {}
    for line in lines:
        if not line.strip(): #  if not line.stripit():
            continue
        if line.startswith('>'):
            label = line.strip()[1:]
            seqs[label] = ""
        else:
            seqs[label] += line.strip() #seqs[label] += line.strip()[1:]
    return seqs
...
def extract_kmers(seqs, k=15, skip_unknown=True):
    ...
    for label, seq in seqs.items(): #for label, seq in seqs.items()
        for i in range(len(seq)-kmer_size+1): #for i in range(len(seq)-kmer_size)
...
def print_stats(kmer_table):
    ...
    """
    #print(list(kmer_table.items())[0:5])
    print("MY OUTPUT")
    res = kmer_table
...
def run_jellyfish(input_fn, kmer_size=15):
    ...
    cmd = 'jellyfish count -m {} -s 1000000 -o {} {}'\
        .format(kmer_size, out_fn, input_fn) #cmd = 'jellyphish count -m {} -s 1000000 -o {} {}'\
...
def main():
    ...
    kmer_len = 15
    kmers = extract_kmers(dna_seqs, skip_unknown=True, k=kmer_len) #kmers = extract_kmers(dna_seqs, skip_unknown=True, k=14)
    print_stats(kmers)
